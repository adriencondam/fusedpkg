...
class GridSearch_Generalised:
    ...
    def crossval_pair_lambdas(
        ...
        for fold_id, train_idx, test_idx, kept_vars in kept_per_fold:
            ...
            if kept_vars:
                # Reuse per-fold fused problem across fused lambdas
                prob, beta, intercept, lam, onehot_cols = self._get_fused_fold_problem(fold_id, train_idx, kept_vars)
            ...
            err_train += poisson_deviance_mu(mu_train, y_train, exposure=exp_train)
            err_test += poisson_deviance_mu(mu_test, y_test, exposure=exp_test)
