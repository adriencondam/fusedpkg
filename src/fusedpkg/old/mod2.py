...
def Compute_Poisson_Deviance(lambdahat, N, v):
    ...
    # n_i * log(n_i / mu_i), with convention 0*log(0/μ)=0
    mask = N > 0
    nlogn = np.zeros_like(N)
    nlogn[mask] = N[mask] * np.log(N[mask] / mu[mask])
...
def initialise_group_lasso_glm(
    data,
    input_var,
    target_var,
    offset_var,
    ...
    n, p = X.shape
    beta = cp.Variable(p)  # model coefficients
    lam = cp.Parameter(nonneg=True, name="lambda_group_full")
    # Linear predictor: η = Xβ + intercept + offset
    eta = X @ beta + intercept + data[offset_var]
    ...
    # Define objective and solve
    # -------------------------------
    objective = cp.Minimize(-log_likelihood + lam * group_penalty)
    prob = cp.Problem(objective)

    return (prob, beta, intercept, lam)
...
def keep_variables_after_group_lasso(
    ...
    temp_modality_var_df = modality_var_df[modality_var_df['variable'].isin(input_variables)].copy()
    print(temp_modality_var_df)
    print(chosen_model_temp)
    # Assign rounded coefficient values to modalities
    temp_modality_var_df["betas"] = [str(round(i, 6)) for i in chosen_model_temp]
...
class GridSearch_Generalised:
    ...
    def fit(self,data,penalty_types,input_variables,target, offset, n_k_fold = 5):
        ...
        self.data_onehot,self.ref_modality_dict,self.input_var_onehot = get_data_ready_for_glm(self.data,
                                                                                               ...
                                                                                               offset,
                                                                                               "First")
        self.cv_splits = list(KFold(n_splits=self.n_k_fold, shuffle=True)
                      .split(self.data_onehot))
        modality_arr=[]
        variables_arr = []
    ...
    def fit_one_lambda(self,data,inputs,lbd_fused,lbd_group):
        ...
        prob, beta, intercept, lam  = initialise_group_lasso_glm(self.data_onehot,
                                                                     self.input_variables,
                                                                     self.target,
                                                                     self.offset,
                                                                     self.penalty_types,
        ...
        lam.value = float(lbd_group)
        prob.solve(solver=cp.CLARABEL, warm_start=True)
        grouped_model_temp = np.concatenate(([intercept.value], beta.value))
    ...
    def crossval_pair_lambdas(self,
        ...
        for (train_idx, test_idx, temp_var_kept) in kept_per_fold:
            ...
            if temp_var_kept != []: 
                # Fit GLM on training fold with fused penalty only
        ...
        # FINAL MODEL on full data (unchanged)
        nb_modalities,total_modality_dict,betas,kept_var = self.fit_one_lambda(
            self.data_onehot, inputs, fused_lambda_val, group_lambda_val
        )
    ...
    def _get_group_lasso_full_problem(self):
        """
        Build (once) and cache a CVXPy problem for the group-lasso GLM on the
        full data and the full list of input variables.

        Lambda (group penalty) is a cp.Parameter so we can reuse the same
        problem for many lambda values with warm starts.
        """
        if hasattr(self, "_group_lasso_full_cache"):
            return self._group_lasso_full_cache

        input_var = self.input_variables
        # One-hot columns for ALL input variables
        input_var_onehot = get_onehot_columns(self.data_onehot, input_var)
        X = self.data_onehot[input_var_onehot].to_numpy()
        Y = self.data_onehot[self.target].to_numpy()
        offset_arr = self.data_onehot[self.offset].to_numpy()
        ...
        if self.family.lower() == "poisson":
            ...
            raise ValueError(f"Unsupported family {self.family} in group-lasso path")

        
        penalty_types = {
            k: v for k, v in self.penalty_types.items()
            if any(k.startswith(prefix) for prefix in input_var)
        }
        one_hot_groups = {
        ...
        group_penalty = 0
        for idx in one_hot_groups.values():
            w = np.sqrt(len(idx))
            group_penalty += cp.norm1(w * beta[idx])

        objective = cp.Minimize(-log_likelihood + lam * group_penalty)
        prob = cp.Problem(objective)

        self._group_lasso_full_cache = (prob, beta, intercept, lam)
        return self._group_lasso_full_cache

    @time_it
    def pick_variables_from_group_lasso(self, grouped_lasso_table, lambda_group_temp, counter,prob, beta, intercept, lam):

        #Solve Group Lasso penalised Likelihood
        lam.value = float(lambda_group_temp)
        prob.solve(solver=cp.CLARABEL, warm_start=True)
        
        grouped_model_temp = np.concatenate(([intercept.value], beta.value))

        # 3) Use the coefficients to group modalities (unchanged logic)
        temp_data, temp_var_kept, temp_var_removed = group_modalities_based_on_betas(
            self.data_onehot,
            self.modality_var_df,
            grouped_model_temp,
    ...
    def _get_fused_full_problem(self, inputs):
        """
        Build (once per inputs set) and cache a CVXPy problem for the fused /
        g_fused GLM on the full data.

        Lambda (fused penalty) is a cp.Parameter so we can reuse the same
        problem for many fused_lambda values for this variable list.
        """
        if not hasattr(self, "_fused_full_cache"):
            self._fused_full_cache = {}

        key = tuple(sorted(inputs))
        if key in self._fused_full_cache:
            return self._fused_full_cache[key]

        data = self.data_onehot
        input_var_onehot = get_onehot_columns(data, inputs)
        X = data[input_var_onehot].to_numpy()
        Y = data[self.target].to_numpy()
        offset_arr = data[self.offset].to_numpy()

        n, p = X.shape
        beta = cp.Variable(p)
        intercept = cp.Variable()
        lam = cp.Parameter(nonneg=True, name=f"lambda_fused_{len(self._fused_full_cache)}")
        ...
        if self.family.lower() == "poisson":
            ...
            raise ValueError(f"Unsupported family {self.family} in fused path")

        # Same group structure as in custom_glm_with_fused_penalty
        penalty_types = {
            k: v for k, v in self.penalty_types.items()
            if any(k.startswith(prefix) for prefix in inputs)
        }
        one_hot_groups = {
            key2: [i for i, x in enumerate(data[input_var_onehot].columns)
                   if x.startswith(key2 + "_")]
            for key2 in penalty_types
        }
        one_hot_groups = {k: v for k, v in one_hot_groups.items() if v}
        ...
        # First: graph size for g_fused groups
        for key2, val in one_hot_groups.items():
            if penalty_types.get(key2) == "g_fused":
                g_fused_graph_size += len(val) + 1

        # Second: build penalties (copying your original logic closely)
        for key2, val in one_hot_groups.items():
            # Per-group reference modality length
            ref_mod_length = len(data)
            for col in input_var_onehot:
                if col.startswith(key2):
                    ref_mod_length -= data[col].sum()

            if penalty_types.get(key2) == "fused":
                # Reference modality
                weight = ((data.iloc[:, val[0]].sum() + ref_mod_length) / len(data)) ** 0.5
                fused_penalty += cp.norm1(weight * (beta[val[0]] - 0))
                # Sequential differences
                if len(val) > 1:
                    for i in range(1, len(val)):
                        weight = (
                            (data.iloc[:, val[i]].sum() + data.iloc[:, val[i-1]].sum())
                            / len(data)
                        ) ** 0.5
                        fused_penalty += cp.norm1(weight * (beta[val[i]] - beta[val[i-1]]))

            elif penalty_types.get(key2) == "g_fused":
                for k_idx in range(len(val)):
                    weight = (len(val) / g_fused_graph_size) * (
                        (data.iloc[:, val[k_idx]].sum() + ref_mod_length) / len(data)
                    ) ** 0.5
                    g_fused_penalty += cp.norm1(weight * (beta[val[k_idx]] - 0))
                    if len(val) > 1:
                        for j in range(k_idx, len(val)):
                            weight = (len(val) / g_fused_graph_size) * (
                                (data.iloc[:, val[k_idx]].sum() + data.iloc[:, val[j]].sum())
                                / len(data)
                            ) ** 0.5
                            g_fused_penalty += cp.norm1(
                                weight * (beta[val[k_idx]] - beta[val[j]])
                            )

        objective = cp.Minimize(-log_likelihood + lam * g_fused_penalty + lam * fused_penalty)
        prob = cp.Problem(objective)

        self._fused_full_cache[key] = (prob, beta, intercept, lam)
        return self._fused_full_cache[key]
    ...
    def get_deviance_one_model(self, results, inputs, fused_lambda_temp, group_lambda_temp, rownb):
        ...
        print(
            f"Based on {inputs} input variables (group lambda = {group_lambda_temp}), "
            f"model with fused lambda = {fused_lambda_temp}"
        )

        # 1) Get / build the parameterised fused problem for this variable list
        prob, beta, intercept, lam = self._get_fused_full_problem(inputs)

        # 2) Solve for this specific fused lambda
        lam.value = float(fused_lambda_temp)
        prob.solve(solver=cp.CLARABEL, warm_start=True)

        temp_glm = np.concatenate(([intercept.value], beta.value))

        # 3) Group modalities after fusion (same as before)
        temp_data, temp_var_kept, temp_var_removed = group_modalities_based_on_betas(
            self.data_onehot,
            self.modality_var_df,
            temp_glm,
        ...
        print(
            f"Based on {inputs} input variables, model with fused lambda = {fused_lambda_temp} "
            f"returns {temp_var_kept} variables"
        )
    ...
    def _precompute_group_selection_per_fold(self, inputs, group_lambda_val):
        """
        For a given set of inputs and group lambda, run the group-lasso once
        per CV fold and store which variables are kept. This is reused for all
        fused-lambda values.
        """
        kept_per_fold = []
        for train_idx, test_idx in self.cv_splits:
            ...
            prob, beta, intercept, lam  = initialise_group_lasso_glm(X_train_cv,
                                                                     inputs,
                                                                     self.target,
                                                                     self.offset,
                                                                     self.penalty_types,
            ...
            print(self.modality_var_df)
            temp_var_kept, temp_var_removed = keep_variables_after_group_lasso(
                X_train_cv,
                self.modality_var_df,
                grouped_model_temp,
                inputs,
    ...
    def get_lambda_curve(self):
        ...
        while not max_lambda_group_reached:
            ...
            grouped_lasso_table = self.pick_variables_from_group_lasso(grouped_lasso_table,
                                                         lambda_group_temp,
                                                         counter,prob, beta, intercept, lam)
            counter +=1
        ...
        for m in range(len(grouped_lasso_uniques)):
            ...
            if input_list_temp !=[]:
                ...
                tested_fused_lambda_list = [
                    results_temp.loc[0,'fused_lambda'] + jump * step
                    for step in range(0, self.smoothness_step + 1)
                ]

                # Precompute group selection once for this (variables, group_lambda)
                kept_per_fold = self._precompute_group_selection_per_fold(
                    input_list_temp,
                    group_lambda_temp
                )
