...
def safe_solve(problem, preferred_solvers=None, **kwargs):
    """
    Try a list of solvers in order until one succeeds.
    """
    if preferred_solvers is None:
        preferred_solvers = [cp.CLARABEL, cp.ECOS, cp.SCS]

    last_err = None
    for s in preferred_solvers:
        try:
            problem.solve(solver=s, **kwargs)
            # Check status – only accept optimal / optimal_inaccurate
    ...
    raise last_err if last_err is not None else cp.SolverError("All solvers failed.")
...
def compute_group_lasso_glm(
    ...
    problem = cp.Problem(objective)
    used_solver = safe_solve(problem)
...
def custom_glm_with_fused_penalty(
    ...
    # Define objective and solve
    # -------------------------------
    # print(f'Log likelihood value : {log_likelihood}, group  penalty term : {g_fused_penalty}, gen fused penalty term : {fused_penalty}, lambda : {lambda_1}')
...
class GridSearch_Generalised:
    ...
    def _run_one_fold_general(
        ...
        """Run one CV fold and return (error_train, error_test)."""

        X_train_cv = self.data_onehot.iloc[train_idx]
        X_test_cv = self.data_onehot.iloc[test_idx]

        # ---- this is basically your current loop body ----
    ...
    def crossval_pair_lambdas(
        ...
        fold_results = Parallel(
            ...
            backend="threading" # avoids pickling self & big DataFrames
        )(
            delayed(self._run_one_fold_general)(
                train_idx,
                test_idx,
                inputs,
                group_lambda_val,
            ...
            for train_idx, test_idx in kf.split(self.data_onehot)
        )

        # fold_results is a list of (error_train, error_test)
        error_train_total = sum(et[0] for et in fold_results)
        error_test_total = sum(et[1] for et in fold_results)
    ...
    def get_lambda_curve(self):
        ...
        for m in range(len(grouped_lasso_uniques)):
            ...
            if input_list_temp !=[]:
                ...
                previous_lambda_non_empty = True
                for fused_lambda_temp in tested_fused_lambda_list:
                    if previous_lambda_non_empty:
                        print('\n Step 2.2 : For selected group and fused lambdas, get cross-val')
                        results,previous_lambda_non_empty = self.crossval_pair_lambdas(results,input_list_temp,group_lambda_temp,fused_lambda_temp,k)
                        k+=1
